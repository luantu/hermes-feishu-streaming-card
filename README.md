...
- Hermes 0.13.0+/0.14.0/0.15.x、`v2026.5.16+` 使用 `gateway_run_013_plus`。
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `HFC_VERSION` | `latest` | 指定安装版本，例如 `v3.6.2` 或 `main` |
...
`setup` 是整合安装器：自动生成配置、检查 Hermes 版本和代码 anchor、把插件安装到 Hermes Gateway 实际运行的 venv Python、安装 hook、启动 sidecar 并做健康检查。它支持 `v2026.4.23` 起的旧版 Hermes，也支持 Hermes 0.13.0+/0.14.0/0.15.x 与 `v2026.5.16+` 新版 anchor；Hermes `VERSION` 可带或不带 `v` 前缀。
...
从 V3.2.x/V3.3.0/V3.4.x/V3.5.x/V3.6.x 升级到 V3.6.2 向后兼容，**单 Profile 配置无需任何修改**。如果 Hermes 使用自己的 venv，升级后请重新跑 `setup` 或 `install`，让插件同时进入 Hermes runtime Python 并刷新 hook。
...
- **Hermes 0.13.0+/0.14.0/0.15.x 升级后无卡片**：先跑 `doctor --config ... --hermes-dir ...`，确认 `hook_strategy` 为 `gateway_run_013_plus`，再按需重新安装 hook。
