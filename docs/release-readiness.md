...
当前发布版本为 `4.6.4`。范围：首次交互回调接线、保留历史的 clarify/approval 按需续答、legacy 同方言回退与恢复、可选阅读预设、有来源证明的通知清理及提交前检查。自动化通过不等于真实客户端验收通过。

全量测试、精确合并 CI、资产校验和公开 tag 普通安装仍是发布门禁。桌面/手机与生产结果单独登记在 [V4.6.4 验收清单](wiki/feishu-acceptance-v4.6.4.md)；未复验的现场问题继续保留原边界。
...
- PEP 517 sdist/wheel、全新 Python 3.12 venv 的普通 wheel `site-packages` 包/distribution `4.4.6`、唯一 Hermes plugin entrypoint、24 个 provenance slices 与 CLI help：**通过**。
- 本机生产 Hermes 0.21.0 runtime venv 加载 4.4.6，官方 patcher 安装后仅 managed `gateway/run.py` 发生预期变化；sidecar/Gateway 重启并达到 `runtime_ready / integrity=safe`：**通过**。
