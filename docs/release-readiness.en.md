...
- PEP 517 sdist/wheel, plus a fresh Python 3.12 regular-wheel install with `site-packages` package/distribution `4.4.6`, one Hermes plugin entrypoint, all 24 provenance slices, and CLI help: **passed**.
- Local production Hermes 0.21.0 loaded 4.4.6 from its runtime venv. After the official patcher install, only managed `gateway/run.py` changed as expected; the restarted sidecar/Gateway reached `runtime_ready / integrity=safe`: **passed**.
