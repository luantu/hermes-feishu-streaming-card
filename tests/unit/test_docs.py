...
def test_v4212_approval_and_zero_tool_timeline_release_contract():
    ...
    notes = read_doc("docs/release-notes-v4.2.12.md")
    notes_en = read_doc("docs/release-notes-v4.2.12.en.md")
    todo = read_doc("TODO.md")
    event_flow = read_doc("docs/wiki/event-flow.md")
    acceptance = read_doc("docs/wiki/feishu-acceptance.md")

    assert 'version = "4.2.12"' in pyproject
    assert '__version__ = "4.2.12"' in package
    assert config.startswith("# Hermes Feishu Streaming Card V4.2.12")
    assert 'HFC_VERSION: "${HFC_VERSION:-v4.2.12}"' in compose
    assert "HFC_VERSION: v4.2.12" in workflow
    assert "当前发布候选为 `4.2.12`" in readiness
    assert "Current release candidate: `4.2.12`" in readiness_en
    assert "allow_custom_input" in notes
    assert "allow_custom_input" in notes_en
    assert "smart_denied" in notes
    assert "zero-tool" in notes_en
    assert "V4.2.12" in todo
    assert "暂无可展示的思考或工具记录" in notes
    assert "interaction.requested" in event_flow
    assert "truthy string" in acceptance
    assert "贡献者证据" in acceptance
...
def test_readme_documents_sidecar_only_and_supported_hermes_version():
    ...
    assert len(readme.splitlines()) <= 281
...
def test_v400_release_docs_cover_live_runtime_cards():
    ...
    for doc in (readme, readme_en, install_doc, guide, guide_en):
        assert "HFC_VERSION=v4.2.12" in doc
    for event_name in (
...
def test_v4021_release_docs_record_content_integrity_and_real_feishu_acceptance():
    ...
    for doc in (readme, readme_en, install_doc, guide, guide_en):
        assert "HFC_VERSION=v4.2.12" in doc
    assert "`v4.2.12`（Compose 示例）" in guide
    assert "The Compose example defaults `HFC_VERSION` to `v4.2.12`." in guide_en
...
def test_v414_release_docs_define_manifestless_legacy_migration_candidate():
    ...
    assert "HFC_VERSION=v4.2.12" in install_doc
    assert "HFC_VERSION: v4.2.12" in workflow
    assert 'HFC_VERSION: "${HFC_VERSION:-v4.2.12}"' in compose
