...
def test_v400_release_docs_cover_live_runtime_cards():
    ...
    assert 'HFC_VERSION: "${HFC_VERSION:-v4.2.9}"' in compose
    for doc in (readme, readme_en, install_doc, guide, guide_en):
        assert "HFC_VERSION=v4.2.9" in doc
    for event_name in (
...
def test_v4021_release_docs_record_content_integrity_and_real_feishu_acceptance():
    ...
    for doc in (readme, readme_en, install_doc, guide, guide_en):
        assert "HFC_VERSION=v4.2.9" in doc
    assert "`v4.2.9`（Compose 示例）" in guide
    assert "The Compose example defaults `HFC_VERSION` to `v4.2.9`." in guide_en
    ...
    assert "当前发布候选为 `4.2.9`" in readiness
    assert "Current release candidate: `4.2.9`" in readiness_en
...
def test_v411_release_docs_define_upgrade_recovery_safety_contract():
    ...
    assert "HFC_VERSION: v4.2.9" in workflow
...
def test_v414_release_docs_define_manifestless_legacy_migration_candidate():
    ...
    assert "HFC_VERSION=v4.2.9" in install_doc
    assert "HFC_VERSION: v4.2.9" in workflow
    assert 'HFC_VERSION: "${HFC_VERSION:-v4.2.9}"' in compose
