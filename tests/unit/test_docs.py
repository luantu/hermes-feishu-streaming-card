...
def test_v400_release_docs_cover_live_runtime_cards():
    ...
    assert 'HFC_VERSION: "${HFC_VERSION:-v4.2.6}"' in compose
    for doc in (readme, readme_en, install_doc, guide, guide_en):
        assert "HFC_VERSION=v4.2.6" in doc
    for event_name in (
...
def test_v4021_release_docs_record_content_integrity_and_real_feishu_acceptance():
    ...
    for doc in (readme, readme_en, install_doc, guide, guide_en):
        assert "HFC_VERSION=v4.2.6" in doc
    assert "`v4.2.6`（Compose 示例）" in guide
    assert "The Compose example defaults `HFC_VERSION` to `v4.2.6`." in guide_en
    ...
    assert "当前发布候选为 `4.2.6`" in readiness
    assert "Current release candidate: `4.2.6`" in readiness_en
...
def test_v411_release_docs_define_upgrade_recovery_safety_contract():
    ...
    assert "HFC_VERSION: v4.2.6" in workflow
...
def test_v414_release_docs_define_manifestless_legacy_migration_candidate():
    ...
    assert "HFC_VERSION=v4.2.6" in install_doc
    assert "HFC_VERSION: v4.2.6" in workflow
    assert 'HFC_VERSION: "${HFC_VERSION:-v4.2.6}"' in compose
...
def test_v426_docs_cover_issue_and_update_release_contracts():
    changelog = read_doc("CHANGELOG.md")
    readme = read_doc("README.md")
    readme_en = read_doc("README.en.md")
    install_doc = read_doc("README-install.md")
    ...
    readiness = read_doc("docs/release-readiness.md")
    readiness_en = read_doc("docs/release-readiness.en.md")
    notes = read_doc("docs/release-notes-v4.2.6.md")
    notes_en = read_doc("docs/release-notes-v4.2.6.en.md")
    todo = read_doc("TODO.md")

    for text in (changelog, readme, readme_en, install_doc, guide, guide_en, todo):
        assert "V4.2.6" in text or "v4.2.6" in text
    assert "当前发布候选为 `4.2.6`" in readiness
    assert "Current release candidate: `4.2.6`" in readiness_en
    assert "## V4.2.6 发布门禁" in readiness
    assert "## V4.2.6 Release Gates" in readiness_en
    ...
    for text in (notes, notes_en):
        for marker in (
            ...
            assert marker in text
    assert "五分钟" in notes
    assert "five minutes" in notes_en
