...
# Contract excerpts from Hermes bf53ff0 (send_final_ledgered ledger bracket);
...
class BasePlatformAdapter:
    ...
    async def _record_delivery_obligation(
        ...
        try:
            ...
            if not await asyncio.to_thread(ledger_enabled):
                return None
            source = event.source
            # ``ledger_message_id`` wins when set: a queued chain's final answers the last
            # message of the chain, not the event that opened it.
    ...
    async def send_final_ledgered(
        ...
        send with retry, finalize from the result. ``event`` supplies the source and the ledger
        identity (``ledger_message_id`` or ``message_id``)."""
